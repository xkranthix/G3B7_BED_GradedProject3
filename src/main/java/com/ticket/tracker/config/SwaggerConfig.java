...
public class SwaggerConfig {
	...
	public Docket employeeAPI() {
		return new Docket(DocumentationType.SWAGGER_2).groupName("Employee Management API").select()
				.apis(RequestHandlerSelectors.basePackage("com.management.employee.controller")).build()
	...
	private ApiInfo apiInfo() {
		return new ApiInfoBuilder().title("Ticket Tracking").description("Ticket Tracking API referance for developers")
				.termsOfServiceUrl("http://tickettracking.com")
				.contact(new Contact("Ticket Tracking API", "http://tickettracking.com", "tickettracking@gmail.com"))
				.license("Ticket Tracking License").licenseUrl("http://tickettracking.com").version("1.0").build();
	}
